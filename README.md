# Modbus water tank testbed

A closed-loop Modbus TCP testbed: a simulated water tank, a C control loop using libplctag, and a browser GUI.

## Components
- `plant.py`: simulates a water tank over Modbus TCP (port 5020). Holding register 0 is tank level, coil 0 is pump on/off.
- `client.c`: on-off controller. Reads tank level, turns the pump on/off via libplctag, writes state to `www/status.json`.
